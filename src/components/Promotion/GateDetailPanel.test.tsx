...
const targetStage: DerivedPipelineStage = {
  ...
  activeChecks: [
    { key: 'argocd-health', phase: 'pending' },
  ],
};

describe('GateDetailPanel', () => {
  it('renders gate title', () => {
    render(<GateDetailPanel gate={gate} targetStage={targetStage} onRetryCheck={jest.fn()} onApproveCheck={jest.fn()} />);
  ...
  it('shows success alert after approve', async () => {
    ...
    render(<GateDetailPanel gate={gate} targetStage={targetStage} onRetryCheck={jest.fn()} onApproveCheck={onApprove} />);
  ...
  it('shows success alert after retry', async () => {
    ...
    render(<GateDetailPanel gate={gate} targetStage={targetStage} onRetryCheck={onRetry} onApproveCheck={jest.fn()} />);
  ...
  it('shows Manual check for checks without URL', () => {
    render(<GateDetailPanel gate={gate} targetStage={targetStage} onRetryCheck={jest.fn()} onApproveCheck={jest.fn()} />);
    expect(screen.getAllByText('Manual check').length).toBeGreaterThanOrEqual(1);
  });
});
